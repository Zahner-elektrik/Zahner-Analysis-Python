...
from zahner_analysis.file_import.impedance_model_import import IsfxModelImport, IsfxModelElement, IsfxModelElementParameter
...
class EisFittingResult:
    ...
    def __init__(self, fitResult: str, fittedModel: IsfxModelImport, fittedSimulatedData: IsmImport, fitInputData: IsmImport):
    ...
    def getFitResultJson(self, fmt="json"):
        ...
        if fmt == "json":
            ...
            return json.dumps(self._fitResult, ensure_ascii=False, sort_keys=True, indent=2)
    ...
    def save(self, path="", foldername="", exist_ok=True,
        ...
        if saveFitResultJson:
            with open(os.path.join(path, fitResultJsonFilename), 'w', encoding='utf-8') as f:
        ...
        if saveFittedSimulatedSamples:
            self._fittedSimulatedData.save(os.path.join(path, fittedSimulatedDataFilename))
...
class EisFitting:
    ...
    def fit(self, model: IsfxModelImport, data: IsmImport=None, fitParams: dict={}, simulationParams: dict={}, timeout: float=None) -> EisFittingResult:
        """ Performing the fit.
        ...
        return EisFittingResult(fitResult, fittedModel, fittedSimulatedSamples, fitInputSamples)
    ...
    def _startFit(self, model: IsfxModelImport, data: IsmImport, params: dict) -> str:
        ...
        files = [
                ...
                ('model-file', (model.getFileName(), io.BytesIO(model.getBinaryFileContent()))),
    ...
    def simulate(self, model: IsfxModelImport, simulationParams: dict, timeout: float=None) -> IsmImport:
        """ Simulate the model.
    ...
    def _waitForJob(self, jobId: str, timeout: float=None):
        ...
        while continueWait:
            ...
            if reply.status_code == 200:
                ...
                continueWait = False
                logging.error("Zahner Analysis reply: " + str(json.loads(reply.content)))
...
class EisFittingPlotter:
    ...
    @staticmethod
    def plotBode(fittingResult: EisFittingResult, impedanceData: IsmImport=None, axes=None, zTogetherPhase=True, absPhase=True):
        ...
        if impedanceData is not None:
            (fig, axes) = bodePlotter(axes, impedanceObject=impedanceData,
                                   zTogetherPhase=zTogetherPhase,
                                   absPhase=absPhase)
        else:
            (fig, axes) = bodePlotter(axes, impedanceObject=fittingResult.getFitInputData(),
                                   zTogetherPhase=zTogetherPhase,
                                   absPhase=absPhase)
        
        (fig, axes) = bodePlotter(axes, impedanceObject=fittingResult.getFittedSimulatedData(),
                               zTogetherPhase=zTogetherPhase,
                               absPhase=absPhase,
    ...
    @staticmethod
    def plotNyquist(fittingResult: EisFittingResult, impedanceData: IsmImport=None, ax=None, minusNyquist=True, maximumAbsImpedance: float=None):
        ...
        if impedanceData is not None:
            (fig, ax) = nyquistPlotter(ax, impedanceObject=impedanceData,
                                      minusNyquist=minusNyquist,
                                      maximumAbsImpedance=maximumAbsImpedance)
        else:
            (fig, ax) = nyquistPlotter(ax, impedanceObject=fittingResult.getFitInputData(),
                                      minusNyquist=minusNyquist,
                                      maximumAbsImpedance=maximumAbsImpedance)
        
        (fig, ax) = nyquistPlotter(ax, impedanceObject=fittingResult.getFittedSimulatedData(),
                                  minusNyquist=minusNyquist,
